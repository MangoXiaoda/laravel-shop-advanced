...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment)
    {
        ...
        {
            // 如果没有支付的还款，原则上不可能，因为如果分期已结清则在上一个判断就退出了
            throw new InvalidRequestException('该分期订单已结清');
        }

        // 调用支付宝的网页支付
        return app('alipay')->web([
            // 支付宝订单号使用分期流水号 + 还款计划编号
            ...
            'return _url'  => route('installments.alipay.return'),    
        ]);
    }
    ...
    public function alipayNotify()
    {
        ...
        Log::error('支付宝支付日志1:'.json_encode($data->trade_status));
        if (!in_array($data->trade_status, ['TRADE_SUCCESS', 'TRADE_FINISHED'])) {
            return app('alipay')->success();
        }

        // 拉起支付时使用的支付订单号是由分期流水号 + 还款计划编号组成的
        // 因此可以通过支付订单号还原出这笔还款是哪个分期付款的还款计划
        Log::error('支付宝支付日志2:'.json_encode($data->out_trade_no));
